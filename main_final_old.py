...
def get_existing_child_tei(session_dhis2, dhis2_api_url, child_unique_id, program_uid="pUOYd7YC9GN"):
    """
    ...
    url = f"{dhis2_api_url}/trackedEntityInstances"
    params = {
        "filter": f"aAMjO9TQK0I:eq:{child_unique_id}",
        "fields": "trackedEntity,orgUnit,enrollments[enrollment,program,orgUnit]",
        "paging": "false",
    }
    ...
    try:
        ...
        response.raise_for_status()
        data = response.json()
        tracked_entities = data.get("trackedEntityInstances", [])
