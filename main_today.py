...
def transform_to_dhis2_events(odk_data, orgUnit_code_uid_dict):
    ...
    for index, submission in enumerate(odk_data):
        ...
        if 'login_check1' not in submission or 'g_info' not in submission:
            print(f"Missing 'login_check1' or 'g_info' in submission: {submission}")
        ...
        execution_date = submission["g_info"]["cdate"]
        block_name = submission["login_check1"]["BLOCK_NAME"]
        district_name = submission["login_check1"]["DISTRICT_NAME"]
        block_code = submission["login_check1"]["BLOCK_CODE"]
        district_code = submission["login_check1"]["DISTRICT_CODE"]
        ...
        if orgunit_uid:
            ...
            if not data_value_exists_in_dhis2(session_post,DHIS2_API_POST_URL,event_id, orgunit_uid) and (event_id != 'None' and event_id != 'null'):
                            ...
                            "events": [
                                {
                                    ...
                                    "dataValues": remove_null_values([
                                        ...
                                        ## new tag ageinweeks is added in place of age_week 13/02/2026
                                        {"dataElement": "SfbOkRY9DpZ", "value": assign_value_if_not_null(
                                            submission["g_assessment"]["ageinweeks"])},    
                                        ...
                                        ## new tag ageinweeks is added in place of age_week 25/06/2026
                                        ...
                                        ### changed in new API again changed as on 25/06/2026
                                        ...
                                        {"dataElement": "DYyANDuDU8h", "value": assign_value_if_not_null(
                                            submission["class_fever"])}, ## new option as added in class_fever -- Malaria unlikely as on 13/02/2026
               ...
               # Loop through each value in dg_sign_values
                ...
                if tracker_payloads:
                    ...
                    push_to_dhis2(DHIS2_API_POST_URL,session_post,tracker_payloads,index+1,temp_patient_id, submission["g_info"]["cdate"])
            else:
...
def push_to_dhis2(dhis2_post_url,session_post,dhis2_events,row,patient_id,event_date):
    try:
        #log_info("Data successfully pushed to DHIS2.")
        #print(f"dhis2_events size {len(dhis2_events)}")
        #print(f"dhis2_post_url {dhis2_post_url}")
        ...
        if response.status_code != 200:
            ...
            print(f"Failed to create event in DHIS2. Status code: {response.status_code}, Event Date {event_date}, Response: {response.content}")
            log_error(f"Failed to create event in DHIS2. Status code: {response.status_code}, Event Date {event_date}, Response: {response.content}")

            # Check for conflict messages in the response
            if 'conflicts' in response.json():
                for conflict in response.json()['conflicts']:
                    print(f"DHIS2 Conflict: {conflict['object']}, {conflict['value']} ")
                    log_error(f"DHIS2 Conflict: {conflict['object']}, {conflict['value']} ")

        else:
            global  event_push_count 
            event_push_count = event_push_count + 1
            #event_id = str(submission["g_info"]["patient_id"])
            ...
            log_info(f"Data successfully pushed to DHIS2. with ODK data Sl.No : {row}, and Patient Id {patient_id}, for Event Date {event_date}, event_push_count {event_push_count}" )
            print(f"Data successfully pushed to DHIS2. with ODK data Sl.No : {row}, and Patient Id {patient_id}, for Event Date {event_date}, event_push_count {event_push_count}" )
            # Log the response
            #log_info(f"Tracker event posted. Response: {response.status_code}, {response.text}")
    except Exception as e:
        print(f"An error occurred while pushing data to DHIS2: {e}")
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        try:
            ...
            orgUnit_code_uid_dict = get_orgUnit_code_uid_dict(session_post)
            print(f"orgUnit_code_uid_dict size {len(orgUnit_code_uid_dict)}")
            log_info(f"orgUnit_code_uid_dict size  -- {len(orgUnit_code_uid_dict)}")
