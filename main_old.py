...
def fetch_odk_data():
    try:
        ...
        # print("Data fetched for:", ODK_API_URL)
...
def fetch_odk_data(skip=0, top=100):
    """
    Fetch data from ODK in batches of 'top' size starting from 'skip'.
    """
    try:
        today_date = datetime.now().strftime("%Y-%m-%d")
        updated_odk_api_url = f"{ODK_API_URL}?$filter=__system/submissionDate ge {today_date}&$skip={skip}&$top={top}"
        
        response = requests.get(updated_odk_api_url, auth=ODK_AUTH)
        if response.status_code == 200:
            if response.json() and "value" in response.json():
                log_info(f"Fetched {len(response.json()['value'])} records starting at {skip}.")
                return response.json()["value"]
            else:
                log_info(f"No data returned for batch starting at {skip}.")
                return []
        else:
...
def main():
    try:
        ...
        batch_size = 100
        while True:
            odk_data = fetch_odk_data(skip=skip, top=batch_size)
            if not odk_data:  # Break the loop if no more data
                log_info("No more data to fetch from ODK. Exiting.")
                break
